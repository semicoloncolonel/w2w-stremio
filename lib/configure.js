module.exports = function configurePage(manifest, host) {
  ...
  const allCheckboxes = (manifest.config || []).filter((c) => c.type === "checkbox");
  const displayOptions = allCheckboxes.filter((c) => c.group === "display");
  const exclusionCheckboxes = allCheckboxes.filter((c) => c.group !== "display");
...
<body>
  <div class="container">
    ...
    <div class="tagline">Use the year and category filters in your Stremio client to narrow down each catalog. By default, the most recent year is shown.</div>

    ${
      displayOptions.length > 0
        ? `<div class="section-label">Display</div>
    <div class="display-options">
      ${displayOptions.map(renderDisplayOption).join("")}
    </div>`
        : ""
    }
