...
<body>
  ...
  <script>
    ...
    // Pre-fill from config in URL path (when opened via Stremio gear icon)
    function prefillFromUrl() {
      try {
        const parts = window.location.pathname.split('/');
        for (const part of parts) {
          const decoded = decodeURIComponent(part);
          if (decoded.startsWith('{') && decoded.includes('tmdbKey')) {
            const config = JSON.parse(decoded);
            if (config.tmdbKey) {
              document.getElementById('tmdbKey').value = config.tmdbKey;
            }
            document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
              if (config[cb.name] === 'on' || config[cb.name] === 'true') {
                cb.checked = true;
              }
            });
            return;
