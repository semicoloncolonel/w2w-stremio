...
const CONFIG_FIELDS = [
  // Display options. Stremio mobile clients only render the `genre` extra as
  // a visible filter — extras named `year`, `skip`, etc. are ignored. Mobile
  // mode folds year options into the genre extra (prefixed "Year: 2024") so
  // mobile users can filter by year. Catalog handler parses the prefix back.
  { key: "mobileMode", type: "checkbox", title: "Mobile mode (combined filter)", group: "display" },
...
// Transform a built manifest for mobile clients. Stremio mobile renders only
// the `genre` extra; year extras are silently dropped. We collapse year
// options into the genre extra with a "Year: <n>" prefix so mobile users
// can pick a year. The catalog handler decodes the prefix back into a year
// filter (see addon.js filterAndPaginate). Web manifests are untouched —
// users install the mobile URL only when they need it.
//
// Constraint: mobile mode lets users pick year OR category at a time, not
// both. That's an inherent Stremio mobile limitation, not a bug here.
