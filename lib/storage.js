// Storage abstraction with three interchangeable backends:
// - "azure": Azure Blob Storage (production)
// - "blob":  Vercel Blob          (legacy production, kept for fallback only)
...
function selectBackend() {
  ...
  let value = explicit;
  if (!value) {
    // Auto-detect by host: Azure Functions sets WEBSITE_INSTANCE_ID; Vercel
    // sets VERCEL=1. Local dev falls through to file.
    if (process.env.AZURE_STORAGE_CONNECTION_STRING || process.env.WEBSITE_INSTANCE_ID) {
      value = "azure";
  ...
  if (value === "azure") return azureBackend;
...
// ---------------------------------------------------------------------------
// Azure Blob backend (lazy require + lazy client)
// ---------------------------------------------------------------------------
//
// Requires AZURE_STORAGE_CONNECTION_STRING in the environment. Container name
// defaults to "catalogs" and can be overridden with AZURE_STORAGE_CONTAINER.

let _azureContainer = null;
function getAzureContainer() {
  if (_azureContainer) return _azureContainer;
  const conn = process.env.AZURE_STORAGE_CONNECTION_STRING;
  if (!conn) {
    throw new Error("AZURE_STORAGE_CONNECTION_STRING not set");
  }
  const { BlobServiceClient } = require("@azure/storage-blob");
  const containerName = process.env.AZURE_STORAGE_CONTAINER || "catalogs";
  _azureContainer = BlobServiceClient.fromConnectionString(conn).getContainerClient(containerName);
  return _azureContainer;
}

const azureBackend = {
  async putJSON(key, data) {
    const block = getAzureContainer().getBlockBlobClient(key);
  ...
  async getJSON(key) {
    ...
    try {
      ...
      // BlobNotFound → treat as missing key, matching file/blob backend semantics.
...
// ---------------------------------------------------------------------------
// Vercel Blob backend (lazy require so the file backend works without the
// package being usable / a token being set)
// ---------------------------------------------------------------------------
